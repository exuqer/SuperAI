      ...
      <template v-else>
        <section class="summary-grid" aria-label="Сводка запуска">
          ...
          <article class="summary-card">
            ...
            <strong>{{ formatNumber(initialSnapshot?.temperature || 0, 3) }} → {{ formatNumber(selectedSnapshot?.temperature || 0, 3) }}</strong>
            <small>конец выбранного шага</small>
          </article>
          <article class="summary-card">
            <span>Энергия</span>
            <strong>{{ formatNumber(selectedSnapshot?.metrics.total_energy || 0) }}</strong>
            <small>{{ selectedSnapshot?.metrics.active_nodes || 0 }} активных · {{ selectedSnapshot?.metrics.evicted_nodes || 0 }} вытесненных</small>
          </article>
          <article class="summary-card">
            <span>Удержание</span>
            <strong>{{ formatPercent(selectedSnapshot?.metrics.average_retention || 0) }}</strong>
            <small>среднее по {{ selectedSnapshot?.nodes.length || 0 }} узлам</small>
          </article>
        </section>
        ...
        <section class="panel timeline-panel">
          ...
          <div class="charts">
            <article v-for="metric in metrics" :key="metric.key" class="trend">
              <div class="trend-head"><span>{{ metric.label }}</span><b>{{ metric.value(selectedSnapshot) }}</b></div>
              <svg viewBox="0 0 300 110" role="img" :aria-label="`${metric.label} по шагам`">
                ...
                <circle
                  ...
                  r="4"
                  @click="selectedStep = snapshot.step"
                />
              </svg>
            </article>
          </div>
          <div class="event-strip" aria-label="События запуска">
            ...
            <button v-for="event in primary.events" :key="String(event.id)" class="event" @click="selectedStep = Number(event.step)">
        ...
        <section class="two-columns">
          <article class="panel candidates-panel">
            ...
            <div v-if="!selectedSnapshot?.candidates.length" class="empty-inline">В выбранном шаге нет сцен-кандидатов.</div>
            <div v-else class="candidate-list">
              <button
                v-for="(candidate, index) in selectedSnapshot.candidates"
          ...
          <article class="panel nodes-panel">
            <div class="section-head">
              <div>
                <div class="kicker">УЗЛЫ ШАГА</div>
            ...
            <div class="node-list">
              <button
                v-for="node in selectedSnapshot?.nodes || []"
...
const hasAnswerRole = computed(() => primary.value?.query_components.some(item => item.word_form_cloud_id === null) || false);

const metrics = [
  { key: 'average_activation', label: 'Активация', value: (snapshot: HiveAnalyticsSnapshotV2 | null) => formatPercent(snapshot?.metrics.average_activation || 0) },
  { key: 'average_retention', label: 'Удержание', value: (snapshot: HiveAnalyticsSnapshotV2 | null) => formatPercent(snapshot?.metrics.average_retention || 0) },
  { key: 'total_energy', label: 'Энергия', value: (snapshot: HiveAnalyticsSnapshotV2 | null) => formatNumber(snapshot?.metrics.total_energy || 0) },
  { key: 'temperature', label: 'Температура', value: (snapshot: HiveAnalyticsSnapshotV2 | null) => formatNumber(snapshot?.temperature || 0, 3) },
];
