...
class SemanticFieldService:
    ...
    def project_query(self, frame: QueryFrame) -> dict[str, Any]:
        ...
        for row in rows:
            ...
            anchors.append({"cloud_id": str(row["id"]), "concept_id": concept, "center": decode(row["center_json"], []), "covariance": decode(row["covariance_json"], []), "field_revision": int(row["field_revision"]), "resolution_source": source, "confidence": 1.0 if source == "CONCEPT_ID" else 0.72 if source == "LEMMA" else 0.35, "query_element_id": str(elements[match_index].get("concept_id") or elements[match_index].get("entity_id") or elements[match_index].get("node_id") or elements[match_index].get("id") or elements[match_index].get("element_id") or terms[match_index]) if match_index < len(elements) and isinstance(elements[match_index], Mapping) else terms[match_index]})
        ...
        return {"anchor_clouds": anchors, "positive_gradients": [{"cloud_id": item["cloud_id"], "weight": item["confidence"], "resolution_source": item["resolution_source"]} for item in anchors], "negative_gradients": [{"value": item, "type": "REPULSION_CONSTRAINT", "excluded_region": True} for item in negatives], "relation_projections": [{"relation": gap.expected_relation, "gap_id": gap.gap_id, "weight": 0.5} for gap in frame.gaps if gap.expected_relation], "gap_field": {"gap_ids": [gap.gap_id for gap in frame.gaps]}, "temporal_projection": frame.temporal_scope or {}, "context_projection": {"known_elements": list(frame.known_elements), "contextual": bool(frame.context_inheritance.get("mode") == "ALLOW")}, "active_dimensions": active_dimensions, "field_region": {"region_id": f"region:{frame.query_id}", "center": center, "radius": 0.75, "active_dimensions": active_dimensions, "field_revision": max((item["field_revision"] for item in anchors), default=0)}}
